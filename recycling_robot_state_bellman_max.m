...
% 
MAX_N_ITERS = 100000; iterCnt = 0; 
CONV_TOL    = 1e-10; delta = 1e10; 


while( ( (delta > CONV_TOL) && (iterCnt <= MAX_N_ITERS) ) || all(abs(T(:)-V(:))~=0) ) 
  T = V; 
  % update states from high to empty : 
  
  V(1) = max( [ 1*( Rwait + gamma*V(1) ), alpha*( Rsearch + gamma*V(1) ) + (1-alpha)*( Rsearch + gamma*V(2) ) ] );  
    
  V(2) = max( [ 1*( -0.5 + gamma*V(1) ), 1*( Rwait + gamma*V(2) ), beta*( Rsearch + gamma*V(2) ) + (1-beta)*( Rsearch + gamma*V(3)) ] );  
    
  V(3) = max( 1* ( 0 + gamma*V(3)) );  
